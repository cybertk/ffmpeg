  ...
  'conditions': [
    ['OS != "win" and use_system_ffmpeg == 0 and build_ffmpegsumo != 0', {
      'variables': {
        ...
        'platform_config_root': 'config/<(ffmpeg_branding)/<(os_config)/<(ffmpeg_config)',
      },
      'includes': [
    ...
    },
    {
      ...
      'variables': {
        ...
        'yasm_flags': [
          ...
          '>@(more_yasm_flags)',
          '-I', '<(platform_config_root)',
          '-I', '<(ffmpeg_source_dir)/libavcodec/x86/',
          '-I', '<(ffmpeg_source_dir)/libavutil/x86/',
          '-I', '<(ffmpeg_source_dir)',
