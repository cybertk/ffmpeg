...
# Flags that are used only in AvaConverter.
function add_flag_avaconverter {
  FLAGS_AVACONVERTER="$FLAGS_AVACONVERTER $*"
}
...
function build {
  ...
  if [[ "$HOST_OS" = "$TARGET_OS" && "$CONFIG_ONLY" = "" ]]; then
    ...
    LIBS="libswscale/$(dso_name swscale $LIBSWSCALE_VERSION_MAJOR) $LIBS"
    LIBS="libswresample/$(dso_name swresample $LIBSWRESAMPLE_VERSION_MAJOR) $LIBS"
    for lib in $LIBS; do
...
# Disable command line programs.
add_flag_common --disable-programs

# Disable all hardware acceleration codecs.
add_flag_common --disable-hwaccels
add_flag_common --enable-hwaccel=h264_vda
...
# AvaConverter specific configuration.
add_flag_avaconverter --enable-encoder=libx264
add_flag_avaconverter --enable-encoder=libfaac
add_flag_avaconverter --enable-muxer=mp4
# mp4 depends on rtp.
...
# Enable faac
add_flag_avaconverter --enable-libfaac
add_flag_avaconverter --enable-nonfree

# Enable file protocol, to open file.
add_flag_common --enable-protocol=file
